## intial data, libraries and functions ----
...
## transfrom data (create dummy variable)
glm_f = f
levels(glm_f$Job)
...
## dtree performance
train_res = predict(dtree, train, type="class")
dtree.train.perf = table(train$CarInsurance, train_res, dnn=c("Actual", "Predicted"))
test_res = predict(dtree, test, type="class")
dtree.test.perf = table(test$CarInsurance, test_res, dnn=c("Actual", "Predicted"))
